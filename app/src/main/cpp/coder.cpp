...
int Coder::getModuleSideSize()
{
	return this->moduleSideSize;
}
/*
...
cv::Vec3i getCornerModuleAverage(cv::Mat image, int startX, int startY)
{
	int moduleSideSize = 100;
	...
	cv::Vec3i average;
	average[0] = bSum / (moduleSideSize * moduleSideSize);
	average[1] = gSum / (moduleSideSize * moduleSideSize);
	average[2] = rSum / (moduleSideSize * moduleSideSize);
	return average;
}

cv::Mat Coder::gradientBrightness(cv::Mat image)
{
	cv::Vec3i cornerModuleAverage[4];
	...
	averageClosenessToMaxLuminance[0] = getAverageClosenessToMaxLuminance(cornerModuleAverage[0][0], cornerModuleAverage[0][1], cornerModuleAverage[0][2]);
	averageClosenessToMaxLuminance[1] = getAverageClosenessToMaxLuminance(cornerModuleAverage[1][0], cornerModuleAverage[1][1], cornerModuleAverage[1][2]);
	averageClosenessToMaxLuminance[2] = getAverageClosenessToMaxLuminance(cornerModuleAverage[2][0], cornerModuleAverage[2][1], cornerModuleAverage[2][2]);
	averageClosenessToMaxLuminance[3] = getAverageClosenessToMaxLuminance(cornerModuleAverage[3][0], cornerModuleAverage[3][1], cornerModuleAverage[3][2]);
...
{
	std::vector<cv::Point2i> inputQuad = getAnglesFromImageAlternatively(mat);
    ...
    corners.push_back(mat.rows);
    corners.push_back(mat.cols);
	return corners;
}
...
cv::Mat Coder::checkPicture(cv::Mat image)
{
    return image;
}